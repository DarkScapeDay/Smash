...
public class A_StarSearch implements CommandExecutor
{
    ...
    private ArrayList<Block> getNeighbors(HashMap<Block, Block> blocksFrom, Block b, int height, int width, boolean allowParkour, boolean allowFlight)
    {
        return getNeighbors(blocksFrom, null, b, height, width, allowParkour, allowFlight);
    }

    private ArrayList<Block> getNeighbors(HashMap<Block, Block> blocksFrom, HashMap<Block, Material> origMats, Block b, int height, int width, boolean allowParkour, boolean allowFlight)
    {
    ...
    private boolean isClimbable(Block b, HashMap<Block, Material> origMats)
    {
    ...
    private void addPathBlock(HashMap<Block, Block> blocksFrom, HashMap<Block, Material> origMats, Block b, Block blockToAdd, ArrayList<Block> list, int height, int width, boolean allowParkour, boolean allowFlight)
    {
        width = width/2;
...
//            if (!isClimbable(blockToAdd.getRelative(0, -2, 0).getType(), outputtingPath) && !isClimbable(blockToAdd.getRelative(0, -1, 0).getType(), outputtingPath))
//            {
//                return;
//            }
//            boolean hasPlatform = isClimbable(blockToAdd.getType(), outputtingPath) || isClimbable(blockToAdd.getRelative(0, -1, 0).getType(), outputtingPath) || movementV.getBlockY() == -1;
//            for (int i = -1; !hasPlatform && i < 2; i += 2)
//            {
//                Material possiblePlatform = blockToAdd.getRelative(i, -1, 0).getType();
//                Material possibleSpace = blockToAdd.getRelative(i, 0, 0).getType();
//                if (isClimbable(possiblePlatform, outputtingPath) && !possibleSpace.isSolid())
//                {
//                    hasPlatform = true;
//                }
//            }
//            for (int k = -1; !hasPlatform && k < 2; k += 2)
//            {
//                Material possiblePlatform = blockToAdd.getRelative(0, -1, k).getType();
//                Material possibleSpace = blockToAdd.getRelative(0, 0, k).getType();
//                if (isClimbable(possiblePlatform, outputtingPath) && !possibleSpace.isSolid())
//                {
//                    hasPlatform = true;
//                }
...
//            }
    ...
    public ArrayList<Block> findShortestPath(Block start, Block destination, int height, int width)
    {
    ...
    //A*
    public ArrayList<Block> findShortestPath(Block start, Block destination, int height, int width, boolean allowParkour, boolean allowFlight)
    {
    ...
    //A*
    public void showAStarAlgorithm(final CommandSender commandSender, final Block start, final Block destination, final int height, final int width, final boolean allowParkour, final boolean allowFlight)
    {
        if (start.getWorld() != destination.getWorld())
        {
            Bukkit.getConsoleSender().sendMessage(ChatColor.RED + "Error! Please use shortest path correctly!");
    ...
    private void outputBlockPath(CommandSender commandSender, final ArrayList<Block> returnResult)
    {
