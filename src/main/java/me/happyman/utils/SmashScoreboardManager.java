...
public class SmashScoreboardManager
{
    ...
    public static final String FINAL_SCORES = ChatColor.GOLD + "   " + ChatColor.BOLD + "Final Scores";
    public static HashMap<World, Scoreboard> scoreboards;
    ...
    public static Integer getTimeRemaining(World w)
    {
    ...
    public static String getTimeRemaining(int timeRemaining)
    {
    ...
    public static void setTimeRemaining(World w, int timeLeft)
    {
        if (SmashWorldManager.isSmashWorld(w) && SmashWorldManager.gameHasStarted(w))
        {
            ...
            {
                SmashWorldManager.sendMessageToWorld(w, ChatColor.GOLD + " " + timeLeft);
                playCountdownSound(w.getPlayers());
            }
            if (!SmashWorldManager.isDeathMatchWorld(w))
            {
    ...
    public static String getScoreboardTitle(World w)
    {
        return scoreObjectives.get(w).getDisplayName();
    }

    public static void setScoreboardTitle(World w, String newTitle)
    {
        scoreObjectives.get(w).setDisplayName(newTitle);
    }

    public static Integer getStartTime(World w)
    {
    ...
    public void addStartTime(World w, int seconds)
    {
        if (seconds >= 0)
        {
    ...
    public static void setStartTime(World w, int seconds)
    {
     ...
     * @param seconds
     */
    public static void subtractStartTime(World w, int seconds)
    {
        ...
        else
        {
            ...
            if (newTime > 0)
            {
                ...
                {
                    playCountdownSound(w.getPlayers());
                    SmashWorldManager.sendMessageToWorld(w, ChatColor.GREEN + " " + newTime);
                }
            }
            ...
            {
                SmashWorldManager.sendMessageToWorld(w, ChatColor.YELLOW + "There must be " + SmashWorldManager.getStartRequirement(w) + " players in order for this game to start! :(", false);
                resetStartTime(w);
            }
        }
    }

    public static void initializeScoreboard(final World w)
    {
        if (w != null && !scoreboards.containsKey(w))
        {
            ...
            setScoreboardTitle(w, ChatColor.GOLD + "" + ChatColor.BOLD + "    Map: " + SmashWorldManager.getMapName(w) + "  ");
    ...
    public static void resetStartTime(World w)
    {
    ...
    public static void removeScoreboardIfThere(World w)
    {
        if (scoreboards != null)
        {
    ...
    public static void clearEntries(World w)
    {
    ...
    public static void updateScoreboard(World w)
    {
    ...
    public static String getPlayerEntryName(Player p)
    {
        return getPlayerEntryName(p.getName());
    }

    public static String getPlayerEntryName(String p)
    {
        return ChatColor.AQUA + p;
    }

    public static Integer getScoreValue(World w, String scoreName)
    {
    ...
    public static Integer getPlayerScoreValue(String p, World w)
    {
    ...
    public static void setScoreValue(World w, String scoreName, int score)
    {
    ...
    public static void setPlayerScoreValue(Player p, int score)
    {
        setPlayerScoreValue(p.getName(), p.getWorld(), score);
    }

    public static void setPlayerScoreValue(String p, World w, int score)
    {
        setScoreValue(w, getPlayerEntryName(p), score);
    }

    public static boolean isOnScoreboard(Player p)
    {
        return hasScoreboard(p.getWorld()) && getScoreboardEntries(p.getWorld()).contains(p.getName());
    }

    public static void removeEntry(World w, String entry)
    {
